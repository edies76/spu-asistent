  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Registro de asistencias</h1>
          {!cargando && (
            <p className="mt-0.5 text-sm text-slate-500">
              {sesiones.length} sesion{sesiones.length !== 1 ? "es" : ""} · {totalEstudiantes} registro{totalEstudiantes !== 1 ? "s" : ""}
              {hayFiltros && <span className="ml-1 text-brand-600">· filtrado</span>}
            </p>
          )}
        </div>
      ...
      {filtrosOpen && (
        <div className="animate-fadein overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
            <p className="text-sm font-semibold text-slate-700">Filtrar resultados</p>
            {hayFiltros && (
      ...
      {cargando ? (
        ...
        <EmptyState mensaje="No hay sesiones con los filtros seleccionados." />
      ) : (
